...
def test_registry_known():
    cfg = PackageConfig(product="tiancheng", version="1", artifact_version="1", package_format="tar.gz")
    assert get_installer_class(cfg) is TianchengTarGzInstaller


def test_registry_unknown_raises():
    cfg = PackageConfig(product="unknown", version="1", artifact_version="1", package_format="tar.gz")
